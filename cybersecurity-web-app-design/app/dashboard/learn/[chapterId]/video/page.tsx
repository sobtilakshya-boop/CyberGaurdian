...
export default function VideoSectionPage() {
  ...
  const { markVideosDone, isChapterUnlocked, getChapterProgress } = useProgress()
  ...
  function handleCompleteVideos() {
    markVideosDone(chapterId)
    router.push(`/dashboard/learn/${chapterId}/comic`)
  }
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 backdrop-blur-xl flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-1.5 font-mono text-xs">
              {sections.map(section => {
                ...
                const isActive = section.name === 'Video Labs'
                return (
                  <Link
                    key={section.name}
                    href={section.href}
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
          <div className="flex flex-col gap-1.5">
            ...
            <h2 className="font-mono text-xl font-bold text-white uppercase tracking-wider">
              {chapter.title} - Video Labs
            </h2>
            <p className="font-mono text-xs text-slate-500">
              Watch curated guide videos covering the chapter topics. Expand to expert telemetry guides.
            </p>
          </div>

          <VideoGrid
            videos={chapter.videos}
            onAllWatched={handleCompleteVideos}
          />

          <div className="flex justify-between border-t border-slate-950 pt-6">
            <Link href={`/dashboard/learn/${chapterId}`}>
