...
import { ArrowLeft, BookOpen, Video, Book, Gamepad, Puzzle, FileCheck2, CheckCircle, Award } from 'lucide-react'

export default function ChapterQuizSectionPage() {
  const params = useParams()
  const router = useRouter()
  const { markChapterQuizPassed, isChapterUnlocked, getChapterProgress } = useProgress()
  ...
  function handlePassQuiz(score: number) {
    markChapterQuizPassed(chapterId, score)
  }
  ...
  return (
    <div className="flex flex-col gap-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 backdrop-blur-xl flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-1.5 font-mono text-xs">
              {sections.map(section => {
                ...
                const isActive = section.name === 'Validation Assessment'
                return (
                  <Link
                    key={section.name}
                    href={section.href}
        ...
        {/* Right Side Quiz & Incident News Area */}
        <div className="lg:col-span-3 flex flex-col gap-8">
          <div className="flex flex-col gap-1.5">
            <span className="font-mono text-[10px] text-cyan-400 uppercase tracking-widest">
              Chapter Knowledge Validation
            </span>
            <h2 className="font-mono text-xl font-bold text-white uppercase tracking-wider">
              {chapter.title} - Final Assessment
            </h2>
            <p className="font-mono text-xs text-slate-500">
              Clear this randomized 15-question test with a score of 70% or higher to unlock the next chapter and receive XP.
            </p>
          </div>

          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            {/* The Quiz Engine Box */}
            <div className="xl:col-span-2 rounded-2xl border border-slate-800/80 bg-slate-900/40 p-6 backdrop-blur-xl">
              <QuizEngine
            ...
            {/* Right side threat advisories news feed */}
            <div className="xl:col-span-1 flex flex-col gap-4">
              <h3 className="font-mono text-xs font-bold text-white uppercase tracking-widest mb-1">
                ⚠️ Threat Intel Advisories
              </h3>
          ...
          <div className="flex justify-between border-t border-slate-950 pt-6">
            <Link href={`/dashboard/learn/${chapterId}/puzzles`}>
            ...
            {progress.chapterQuizPassed && (
              <Link href="/dashboard/learn">
                <button className="flex items-center gap-2 px-6 py-3 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-mono text-xs font-bold uppercase tracking-widest transition-all cursor-pointer shadow-[0_0_15px_rgba(6,182,212,0.2)]">
