...
export default function PuzzlesSectionPage() {
  ...
  function handleCompletePuzzle() {
    ...
    // Go to next subsection: final assessment quiz
    router.push(`/dashboard/learn/${chapterId}/quiz`)
  }
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 backdrop-blur-xl flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-1.5 font-mono text-xs">
              {sections.map(section => {
                ...
                const isActive = section.name === 'Security Puzzles'
                return (
                  <Link
                    key={section.name}
                    href={section.href}
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
          <div className="flex flex-col gap-1.5">
            <span className="font-mono text-[10px] text-cyan-400 uppercase tracking-widest">
              Awareness Vocabulary Puzzles
            </span>
            <h2 className="font-mono text-xl font-bold text-white uppercase tracking-wider">
              {chapter.title} - Security Word Search
            </h2>
            <p className="font-mono text-xs text-slate-500">
              Locate the hidden security terms inside the matrix to demonstrate understanding.
            </p>
          </div>
          ...
          <PuzzleShell
            ...
            isComplete={puzzleCompleted || progress.comicQuizPassed} // lets say matching/puzzle completion uses a common trigger or acts as a step
          />

          <div className="flex justify-between border-t border-slate-950 pt-6">
            <Link href={`/dashboard/learn/${chapterId}/activities`}>
